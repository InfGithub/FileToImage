...
def encode(ui: UI):
    ...
    try:
        ...
        file_data_length: int = len(file_data)
        ...
        file_data_length_data: bytes = file_data_length.to_bytes(5) # 5 bytes
        md5_checksum_data: bytes = md5_checksum(file_data).digest() # 16 bytes
        headers: list[bytes] = [
            author_data, version_data, filename_data_full,
            filename_data_length_data, md5_checksum_data, file_data_length_data
        ]
        header_length: list[int] = [len(item) for item in headers]
        header_length_sum: int = sum(header_length)
        data_length_sum: int = header_length_sum + file_data_length + 16
        
        ui.set_loading_info("运行进度：正在编码信息。")
        side_length: int = get_sqrt((data_length_sum + 3) // 4)
        target_size: int = side_length * side_length * 4 - 16
        file_data[:0] = b"\x00" * header_length_sum
        offset: int = 0
        for index, item in enumerate(headers):
            file_data[offset:offset + header_length[index]] = item
            offset += header_length[index]
        file_data.extend(b"\x00" * (target_size - file_data_length))
        result_arr: np.ndarray = np.frombuffer(file_data, dtype=np.uint8)[:target_size]
