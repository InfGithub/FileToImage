...
def encode(ui: UI):
    ...
    try:
        ...
        total_length: int = header_total + data_rb_length
        
        ui.set_loading_info("运行进度：正在编码信息。")
        side_length: int = get_sqrt((total_length + 3) // 4)
        target_size: int = side_length * side_length * 4
        ...
        result_arr: np.ndarray = np.frombuffer(data_rb, dtype=np.uint8)[:target_size].reshape(side_length, side_length, 4)
        
        ui.set_loading_info("运行进度：正在映射图像。")
        result_image: Image.Image = Image.fromarray(result_arr, mode="RGBA")
