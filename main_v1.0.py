...
from hashlib import md5 as md5_checksum
...
class UI:
    def __init__(self):
        ...
        self.root.geometry("250x150+648+648")
...
def encode(ui: UI):
    ...
    try:
        ...
        filename: str = path.basename(Config.file_path)
        filename_rb: bytes = filename.encode("utf-8")
        data_rb_length_rb: bytes = str(len(data_rb)).encode("utf-8")
        md5_checksum_value: str = md5_checksum(data_rb).hexdigest()
        md5_checksum_rb: bytes = md5_checksum_value.encode("utf-8")
        data_rb_img: bytes = Config.spliter.join([version_rb, filename_rb, data_rb_length_rb, md5_checksum_rb, data_rb])

        ui.set_loading_info("运行进度：正在编码信息。")
        data_rb_flat_arr: np.ndarray = np.frombuffer(data_rb_img, dtype=np.uint8)
        side_length: int = get_sqrt(data_rb_flat_arr.shape[0] // 4)
        target_shape: list[int] = [side_length, side_length, 4]
        result: np.ndarray = np.zeros(target_shape, dtype=np.uint8)
        flat: np.ndarray = result.flatten()
        flat[:len(data_rb_flat_arr)] = data_rb_flat_arr
        result: np.ndarray = flat.reshape(target_shape)

        ui.set_loading_info("运行进度：正在映射图像。")
        result_image: Image.Image = Image.fromarray(result)

        ui.set_loading_info("运行进度：正在保存文件。")
        filename_without_ext: str = path.splitext(filename)[0]
        filename_within_png_ext: str = f"{filename_without_ext}.png"
        save_file_path: str = path.join(Config.dir_path, filename_within_png_ext)
        result_image.save(save_file_path)
...
class Config:
    ...
    icon_b64_data: str = "iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAIAAACQkWg2AAAACXBIWXMAAABHAAAARAAbKpbnAAADG0lEQVR4nAEQA\
+/8AOCoccyUXd+pf8mTaYyEgY+HhIq813qsx6LDyrrb4tmlf7+LZdCJV9GKWNKEVc1/UADhpW3QlFzDil7co3eSoqZ4iIyTnK+EjaB0iJOitsHO0L\
zGyLTr0q7t1LDn3cbo3scA6a+Do2k9nVdBoFpEh4aQkZCau5aH2bSlmZeanJqdx6+pmoJ8zG1Lz3BOwI1xxZJ2AOqrgqVmPapYRLFfS4Fwdo59g9q4o\
fPRusvDwK6mo7KssnFrcchePt91VdVuR9VuRwDjonm2dUyRZWCQZF+vk4fCpprasIr3zafWzcrGvbqQnq15h5bHYETXcFTScE/WdFMA3JtzuHd\
PkmNdypuV49PIuamejoSHm5GUj4aLmZCVgJ20bImgil1efE9QhVBOe0ZEAOeVca9dOdVDNthGOZqPmYuAipSLknJpcIKLlqKrtpW0y36dtF\
lnhkNRcE9OZEtKYADfmXmTTS2nLSa4PjeHgIm1rrfSz8rc2dTg5ua9w8PSs7qSc3qCeqNqYotXV3ViYoAA2JuCiUwzfygln0hFypaMtYF3ubizx8b\
BxsXEo6Khr1RTn0RDnmyPkmCDimeHmXaWANWUeqxrUYEpK4QsLrZMRZAmH5F0a+3Qx9vZ2pqYmZE9PpZCQ69zjrV5lK99orWDqADHk322gmykW1mTSkiSR0i\
tYmOpoZ7p4d7W5urY6Oy6n5y6n5zVrKrYr63Woa3JlKAA14RzrltKy2VavlhNb1JOzbCs6/Tx2uPgrr3HipmjwqidxKqfvJmM2b\
ap2q6tzaGgANKGdaVZSLhIPrBANm9DQsqendTo6cnd3svCw56Vlsallbybi8Sop7+josyfoseanQC/j4Sre3DDjGjFjmqHgHatppzB2OKftsCnoaPJw8X\
k29xqYWI3RFhVYnbNsq3hxsEAt5WOknBplmpFsoZh1ap/tYpfoLfIobjJpYeB2bu1pKKjRkRFYWluhIyRgV9gspCRAIx2b2tVTlJCQnNjY4ZwaWhSS0\
xjcWZ9i4uSn5CXpFJQVURCR2VlZ7a2uI19f1lJS0nSufWWhUvLAAAAAElFTkSuQmCC"
